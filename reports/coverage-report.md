...
- **GATE v2 (R18/D-003 — the governing metric): 6/54 = 11.1%** = (full_pass + recipe_verified) / 54 ATTAINABLE Terrain+Veg wave-1 niches. Threshold **40%** (final verdict after L5b, R19). `partial` and `recipe_unverified` do NOT count in v2.
...
- _v1 (legacy, all-present denom): 9/59 = 15.3% (5 full-pass + 3 partial + 1 recipe✓; 5 recipe claims not counted)._
