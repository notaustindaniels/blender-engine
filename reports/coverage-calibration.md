...
| `free_recipe` | 35 | 59% | no dedicated tool, but a plausible free recipe (built-ins/GN/free packs) |
| `paid_only` | 3 | 5% | tools exist but only paid (excluded by the free constraint) |
| `none` | 3 | 5% | no known free tool AND no plausible free recipe |
| **ATTAINABLE (free tool or recipe)** | **53** | **90%** | verified_free + free_tool + free_recipe |
| **NOT attainable (paid/none)** | **6** | **10%** | the true free-ecosystem ceiling |

**Read:** ~**90%** of the 59 wave-1 Terrain+Veg niches have SOME free path; only **10%** (6 niches) appear genuinely unreachable for free (marine flora skews paid: coral/kelp/anemone; plus exotic karst/atoll/meander). So a 40% target is **not** obviously unreachable — but our *current metric* (dedicated-add-on-only, probe-verified) undercounts the attainable base by design.

## Not-attainable niches (the real ceiling)

- `karst_formation` (none, medium) — searched karst/sinkhole/sea-stack Blender: no free tool, no obvious recipe
- `coral_atoll_generator` (none, medium) — no free atoll generator; coral tools are paid (Superhive)
- `meander_braid_tools` (none, low) — no free meander/oxbow tool found; specialized hydrology
...
- `anemone_generator` (paid_only, medium) — bundled in paid Procedural Coral; no standalone free anemone tool
...
1. **Keep 40% but measure against the attainable base.** Redefine the coverage denominator as attainable niches (~53), not all present niches. Then the target asks "of the niches a free path exists for, how many have we verified" — a truer feasibility question. Requires R14 recipes to be probe-verified and the native L2/L5 harvest to run.
2. **Keep 40% of all present niches, unchanged**, and treat the gap as a harvest-completeness signal: the 10 `free_tool` niches need L2-native + GN-pack/L5 sources, the 35 `free_recipe` niches need R14 recipes machine-checked. Coverage should climb toward ~90% as those land — re-evaluate then.
