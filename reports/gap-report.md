# Per-category coverage + gap summary (D-008 R47) — all 26 categories

**Whole-taxonomy coverage: 37/328 = 11.3%** (full_pass + recipe_verified). Per-category detail in `reports/gaps/<category>.md`. A gap is an uncovered niche; attainability from the R15 audit.

| category | covered | present | % | gaps |
|---|---|---|---|---|
| Vegetation & organic | 14 | 25 | 56.0% | 11 |
| Materials & texturing | 7 | 22 | 31.8% | 15 |
| Animation: stylized / NPR | 1 | 4 | 25.0% | 3 |
| Terrain & landscape | 9 | 39 | 23.1% | 30 |
| Fabric & soft goods | 1 | 5 | 20.0% | 4 |
| Animation: cloth, rope & soft proxies | 1 | 5 | 20.0% | 4 |
| Animation: camera & cinematic | 1 | 5 | 20.0% | 4 |
| Characters & creatures | 1 | 8 | 12.5% | 7 |
| Abstract, mograph & design | 1 | 14 | 7.1% | 13 |
| Nature elements & FX geometry | 1 | 24 | 4.2% | 23 |
| Cities & urban | 0 | 18 | 0.0% | 18 |
| Buildings & architecture | 0 | 17 | 0.0% | 17 |
| Rooms & interiors | 0 | 12 | 0.0% | 12 |
| Hard surface & props | 0 | 17 | 0.0% | 17 |
| Simulation-adjacent | 0 | 15 | 0.0% | 15 |
| Emergent formation (agents/particles self-organize into targets) | 0 | 14 | 0.0% | 14 |
| Diegetic data visualization (charts as in-world objects) | 0 | 16 | 0.0% | 16 |
| Physical-process rendering (force-of-nature and craft draw the math) | 0 | 6 | 0.0% | 6 |
| Light & shadow as data | 0 | 3 | 0.0% | 3 |
| Animation: motion fundamentals | 0 | 6 | 0.0% | 6 |
| Animation: character & creature | 0 | 12 | 0.0% | 12 |
| Animation: mechanical | 0 | 9 | 0.0% | 9 |
| Animation: nature & environment | 0 | 8 | 0.0% | 8 |
| Animation: growth, reveal & build | 0 | 10 | 0.0% | 10 |
| Animation: FX & particles | 0 | 6 | 0.0% | 6 |
| Animation: data-driven & utility | 0 | 8 | 0.0% | 8 |
