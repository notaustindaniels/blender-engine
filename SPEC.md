...
4. **R13 native amd64 probe path — proposal pending owner sign-off (amends §1, §5.2).** Emulation crashes cap L2 verification; a native amd64 sandbox is approved in principle but the *path* needs owner approval (GitHub Actions at \$0 preferred). Proposal in `reports/native-probe-proposal.md`; no build until approved.
